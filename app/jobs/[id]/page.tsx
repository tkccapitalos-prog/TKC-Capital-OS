    ...
    <main className="min-h-screen bg-[#050505] p-10 text-white">
      ...
      <section className="mt-8 max-w-4xl">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/10 p-5">Vagas: {job.quantity}</div>
          <div className="rounded-2xl border border-white/10 p-5">Salário: {job.salary || "A combinar"}</div>
        </div>

        <form action="/api/apply" method="post" className="mt-10 grid gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-6">
          <input type="hidden" name="job_id" value={job.id} />

          <input name="full_name" required placeholder="Nome completo" className="rounded-xl bg-white px-4 py-3 text-black" />
          <input name="phone" required placeholder="Telefone / WhatsApp" className="rounded-xl bg-white px-4 py-3 text-black" />
          ...
          <input name="district" placeholder="Cidade / Distrito" className="rounded-xl bg-white px-4 py-3 text-black" />
          <input name="availability" placeholder="Disponibilidade: imediata, 1 semana, etc." className="rounded-xl bg-white px-4 py-3 text-black" />
          <textarea name="experience" placeholder="Experiência / especialidades / obras feitas" className="rounded-xl bg-white px-4 py-3 text-black" />

          <button className="rounded-xl bg-orange-500 px-4 py-3 font-bold text-black">
            Enviar candidatura
          </button>
        </form>
      </section>
