...
async function loadHandovers(supabase) {
  const result = await supabase
    ...
    .select(`
      ...
      updated_at,
      closed_at,
      created_by:operator_profiles!handover_topics_created_by_operator_id_fkey(full_name),
      owner:operator_profiles!handover_topics_owner_operator_id_fkey(full_name),
      handover_acknowledgements(
        operator_id,
        acknowledged_at,
        operator:operator_profiles!handover_acknowledgements_operator_id_fkey(full_name)
      )
    `)
    .order("created_at", { ascending: false })
    .limit(120);

  // Allow the application code to be deployed before the additive database
  // migration. Once the table exists, every other database error remains fatal.
  if (result.error && ["42P01", "PGRST205"].includes(result.error.code)) {
    return [];
  }
  const rows = throwIfError(result, "load_handovers") || [];

  return rows.map((topic) => ({
    id: topic.id,
    propertyCode: topic.property_code,
    sourceDepartment: topic.source_department,
    targetDepartments: topic.target_departments || [],
    title: topic.title,
    details: topic.details,
    location: topic.location || "",
    priority: topic.priority,
    status: topic.status,
    shiftCode: topic.shift_code,
    dueAt: topic.due_at,
    createdByOperatorId: topic.created_by_operator_id,
    createdByName: topic.created_by?.full_name || "Equipe TKC",
    ownerOperatorId: topic.owner_operator_id,
    ownerName: topic.owner?.full_name || "",
    metadata: topic.metadata || {},
    createdAt: topic.created_at,
    updatedAt: topic.updated_at,
    closedAt: topic.closed_at,
    acknowledgements: (topic.handover_acknowledgements || []).map((acknowledgement) => ({
      operatorId: acknowledgement.operator_id,
      operatorName: acknowledgement.operator?.full_name || "Equipe TKC",
...
export async function loadHotelWorkspace(supabase, authUserId) {
  ...
  const [tasks, messages, documents, housekeeping, operators, handovers] = await Promise.all([
...
export async function createHandover(supabase, payload, operatorId) {
  const result = await supabase
    .from("handover_topics")
    .insert({
      property_code: payload.propertyCode || "ibis_nogent",
      ...
      owner_operator_id: payload.ownerOperatorId || null,
      metadata: payload.metadata || { schemaVersion: 1 },
    })
    .select("id")
    .single();
  return throwIfError(result, "create_handover");
}

export async function updateHandoverStatus(supabase, topicId, status) {
  ...
  const result = await supabase
    ...
    .eq("id", topicId);
  throwIfError(result, "update_handover_status");
}

export async function acknowledgeHandover(supabase, topicId, operatorId) {
  const result = await supabase.from("handover_acknowledgements").insert({
    topic_id: topicId,
    operator_id: operatorId,
  });
  if (result.error?.code === "23505") return;
  throwIfError(result, "acknowledge_handover");
}
